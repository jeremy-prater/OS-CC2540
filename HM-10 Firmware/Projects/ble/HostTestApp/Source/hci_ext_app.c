...
static uint8 processExMsgLL( uint8 cmdID, hciExtCmd_t *pCmd);
...
static uint8 processExtMsg( hciPacket_t *pMsg )
{
  ...
  switch( (msg.opCode & 0x380) >> 7 )
  {
    case HCI_EXT_LL_SUBGRP:
      stat = processExMsgLL( (msg.opCode & 0x007F), &msg );
      break;
  ...
  //AddOutgoingMessageBuffer (rspBuf, (6 + rspDataLen));
...
static uint8 processExMsgLL( uint8 cmdID, hciExtCmd_t *pCmd)
{
  ...
  switch( cmdID )
  {
    case HCI_EXT_LL_SET_RX_GAIN:
        stat = HCI_EXT_SetRxGainCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_SET_TX_POWER:
        stat = HCI_EXT_SetTxPowerCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_ONE_PKT_PER_EVT:
        stat = HCI_EXT_SetRxGainCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_CLK_DIVIDE_ON_HALT:
      stat = HCI_EXT_ClkDivOnHaltCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_DECLARE_NV_USAGE:
      stat = HCI_EXT_DeclareNvUsageCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_DECRYPT:
      stat = HCI_EXT_DecryptCmd (&pBuf[0], &pBuf[16]);
      break;
      
    case HCI_EXT_LL_SET_LOCAL_SUPPORTED_FEATURES:
      stat = HCI_EXT_SetLocalSupportedFeaturesCmd(pBuf);
      break;
      
    case HCI_EXT_LL_SET_FAST_TX_RESP_TIME:
      stat = HCI_EXT_SetFastTxResponseTimeCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_MODEM_TEST_TX:
      stat = HCI_EXT_ModemTestTxCmd(pBuf[0], pBuf[1]);
      break;
      
    case HCI_EXT_LL_MODEM_HOP_TEST_TX:
      stat = HCI_EXT_ModemHopTestTxCmd();
      break;
      
    case HCI_EXT_LL_MODEM_TEST_RX:
      stat = HCI_EXT_ModemTestRxCmd (pBuf[0]);
      break;
      
    case HCI_EXT_LL_END_MODEM_TEST:
      stat = HCI_EXT_EndModemTestCmd();
      break;
      
    case HCI_EXT_LL_SET_BDADDR:
      stat = HCI_EXT_SetBDADDRCmd (pBuf);
      break;
      
    case HCI_EXT_LL_SET_SCA:
      stat = HCI_EXT_SetSCACmd (*(uint16*)pBuf);
      break;
      
    case HCI_EXT_LL_ENABLE_PTM:
      stat = HCI_EXT_EnablePTMCmd();
      break;
      
    case HCI_EXT_LL_SET_FREQ_TUNE:
      stat = HCI_EXT_SetFreqTuneCmd (pBuf[0]);
      break;
      
    case HCI_EXT_LL_SAVE_FREQ_TUNE:
      stat = HCI_EXT_SaveFreqTuneCmd();
      break;
      
    case HCI_EXT_LL_SET_MAX_DTM_TX_POWER:
      stat = HCI_EXT_SetMaxDtmTxPowerCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_MAP_PM_IO_PORT:
      stat = HCI_EXT_MapPmIoPortCmd(pBuf[0], pBuf[1]);
      break;
      
    case HCI_EXT_LL_DISCONNECT_IMMED:
      stat = HCI_EXT_DisconnectImmedCmd (*(uint16*)pBuf);
      break;
      
    case HCI_EXT_LL_PER:
      stat = HCI_EXT_PacketErrorRateCmd(*(uint16*)pBuf,pBuf[2]);
      break;
      
    case HCI_EXT_LL_PER_BY_CHAN:
      stat = HCI_EXT_PERbyChanCmd(*(uint16*)pBuf,(perByChan_t *)pBuf + 2); // This will probably have serious errors...
      break;
      
    case HCI_EXT_LL_EXTEND_RF_RANGE:
      stat = HCI_EXT_ExtendRfRangeCmd();
      break;
      
    case HCI_EXT_LL_ADV_EVENT_NOTICE:
      stat = HCI_EXT_AdvEventNoticeCmd(pBuf[0], *(uint16*)(pBuf+1));
      break;
      
    case HCI_EXT_LL_CONN_EVENT_NOTICE:
      stat = HCI_EXT_ConnEventNoticeCmd(pBuf[0], *(uint16*)(pBuf+1));
      break;
      
    case HCI_EXT_LL_HALT_DURING_RF:
      stat = HCI_EXT_HaltDuringRfCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_OVERRIDE_SL:
      stat = HCI_EXT_SetSlaveLatencyOverrideCmd (pBuf[0]);
      break;
      
    case HCI_EXT_LL_BUILD_REVISION:
      stat = HCI_EXT_BuildRevisionCmd(pBuf[0], *(uint16*)(pBuf + 1));
      break;
      
    case HCI_EXT_LL_DELAY_SLEEP:
      stat = HCI_EXT_DelaySleepCmd(*(uint16*)pBuf);
      break;
      
    case HCI_EXT_LL_RESET_SYSTEM:
      stat = HCI_EXT_ResetSystemCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_OVERLAPPED_PROCESSING:
      stat = HCI_EXT_OverlappedProcessingCmd(pBuf[0]);
      break;
      
    case HCI_EXT_LL_NUM_COMPLETED_PKTS_LIMIT:
      stat = HCI_EXT_NumComplPktsLimitCmd(pBuf[0], pBuf[1]);
      break;
...
static uint8 processEvents( osal_event_hdr_t *pMsg )
{
  ...
  if ( msgLen )
  {
    //AddOutgoingMessageBuffer(pBuf,msgLen);
